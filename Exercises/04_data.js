...
//  chapter, which variant do you expect to be useful in more situations? 
...
//  array from a list. Then add a helper function prepend, which takes an element and a list and 
//  creates a new list that adds the element to the front of the input list, and nth, which takes a 
//  list and a number and returns the element at the given position in the list 
...
let deepEqual = (obj1, obj2) => {
    for (let prop in obj1) {
        let hasProp = prop in obj1;
        if(hasProp){
            let type = typeof obj1[prop];
            if(type === 'object'){
                let objPropsAreEqual = deepEqual(obj1[prop], obj2[prop]);
                if(!objPropsAreEqual)
                    return false;
            }
            else {
                let tmp = obj1[prop] === obj2[prop];
                if(!tmp)
                    return false;
            }
        }
